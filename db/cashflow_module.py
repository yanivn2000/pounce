...
def get_items(conn):
    return conn.execute(
        "SELECT id,name,direction,category,amount,currency,frequency,account_id,start_ym,end_ym,notes "
...
def add_item(conn, name, direction, category, amount, currency,
             frequency, account_id, start_ym, end_ym, notes):
    conn.execute(
        "INSERT INTO cashflow_items(name,direction,category,amount,currency,frequency,"
        "account_id,start_ym,end_ym,notes) VALUES(?,?,?,?,?,?,?,?,?,?)",
        [name, direction, category, amount, currency, frequency,
         account_id, start_ym, end_ym or None, notes or None]
    )
    conn.commit()
...
def _to_usd(amount: float, currency: str, usd_nis: float) -> float:
    """Simple conversion — USD passthrough, ILS by explicit rate, others ~1."""
    if currency in ("USD", ""):
        return amount
    if currency == "ILS":
        return amount / usd_nis if usd_nis else 0.0
    # EUR/GBP/CAD etc. — use rough rates; for forecast purposes this is ok
...
def build_forecast(conn, months_ahead: int, usd_nis: float,
    ...
    Returns:
        ...
        forecast  : list of dicts, one per month:
                    { ym, year, month, balances: {id: float}, inflows: float, outflows: float }
    """
    from datetime import date
    today = date.today()
    ...
    for (fy, fm) in forecast_months:
        ...
        if amazon_acc_id and amz_payout:
            ...
            monthly_flows.append({
                ...
                "account_id": amazon_acc_id, "auto": True,
            })
        ...
        for item in items:
            ...
            # Skip Amazon Payout category — handled above automatically
            ...
            acc = next((a for a in accounts if a[0] == account_id), None) if account_id else None
            acc_currency = acc[3] if acc else currency

            # Convert to account currency
            if currency == acc_currency:
                converted = amount
            elif currency == "USD" and acc_currency == "ILS":
                converted = amount * usd_nis
            elif currency == "ILS" and acc_currency == "USD":
                converted = amount / usd_nis if usd_nis else 0
            else:
                converted = amount

            if account_id and account_id in running:
                running[account_id] = running.get(account_id, 0.0) + sign * converted

            monthly_flows.append({
                "name": name, "direction": direction, "category": category,
                "amount": converted, "currency": acc_currency,
                "account_id": account_id, "auto": False,
            })
